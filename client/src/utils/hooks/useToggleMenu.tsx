...
export default function useToggleMenu() {
  ...
  const { data: collapsed } = useQuery<boolean>(ReactQueryKey.TOGGLE_MENU_KEY, () => false, { enabled: false });

  const onToggleMenu = useCallback(() => {
    document.body.classList.toggle('overflow-hidden', collapsed)
    queryClient.setQueryData(ReactQueryKey.TOGGLE_MENU_KEY, (collapsed: any) => !collapsed);
  }, [queryClient]);
